...
| `resolveGameCode({code})` | callable, **unauthenticated** | Uppercase + validate format. Rate limit by IP (below). Look up `codes/{code}`; if found and unexpired, return `{gameId, plan}` where `plan` is the owner's current plan (`"all-access"` or `"free"`; a missing profile counts as free). Else a generic not-found error. |
...
- `resolveGameCode`: per-IP, **100 wrong codes / 1 min; successful lookups are never counted** (changed 2026-09-25 from 30 lookups of any kind / 10 min). A whole school often shares one public IP, so counting every lookup let one class entering the right code lock out the next student for up to 10 minutes, and every lookup writing the same counter doc made class-sized bursts fail on write contention. The counter is *read* before the lookup (no write, so bursts don't contend) and *charged* only after a miss (missing, expired, or stale-catalog code). Once an IP is blocked, even a correct code gets `resource-exhausted`, so the limit can't be used to test whether a code exists. On limit, return `resource-exhausted` with `retryAfter` (always ≤ 60 s) — the play page shows its lockout overlay. A `logger.warn` fires once per window when an IP reaches the limit, so a real school hitting it would show up in logs. The function also runs with `maxInstances: 5` as a bill ceiling for floods; a request Google turns away at that cap arrives as a 429 with no `retryAfter`, which the play page treats as a network hiccup (no attempt counted, no lockout). (Brute force is already mathematically pointless at 33M combinations; this is the abuse backstop.)
...
**Which IP counts.** `X-Forwarded-For` is client-writable, and Express `req.ip` on this stack resolves to the *leftmost* entry — verified attacker-controlled. **Verified in production (2026-09-19):** Google appends exactly **one** entry — the client IP it observed — as the **rightmost** value (a clean request arrived as `66.113.20.76`; a spoofed one as `9.9.9.9,66.113.20.76`; there is no trailing LB entry on gen2 function URLs). The limit therefore keys on the rightmost entry (`GOOGLE_APPENDED_XFF_ENTRIES = 1` in `resolve-code.js`). Keying anywhere left of the appended entries would let an attacker rotate a fake IP per request to bypass the limit, or aim it at a real user's IP; the original plan's assumption of a `<client-ip>, <lb-ip>` two-entry append was wrong for this topology (that shape belongs to the external Application Load Balancer) — re-verify if the functions ever move behind one. An empty header means no Google front end (emulator, direct connection), so the socket address is used instead, and a non-empty-but-short chain logs a warning as a misconfiguration tripwire. IPv6 is bucketed to its /64 prefix, since one subscriber routinely holds a whole /64; anything unparseable falls into a single shared bucket, erring toward over-limiting.
...
- [X] `[MID]` Test full lifecycle with Stripe test clocks: purchase → renew → cancel → lapse.

### Phase 5 — Hardening
- [X] `[HIGH]` Adversarial review: rules, entitlement fields unwritable from clients, rate-limit tuning, probing every callable as a hostile user. *Verdict: rules (tested adversarially in `firestore.rules.test.js`), webhook guards, rebate claims, and share-code entitlement all hold. Two findings fixed below; accepted residual risks: `userPrefs.favorites` elements are unconstrained in rules (rules can't iterate lists; bounded by the 1 MiB doc cap, owner-read-only), malformed `resolveGameCode` input can cost invocations without incrementing the rate counter (accepted; App Check deferred), authenticated callables have no per-user limits (see deferred item below), and parallel checkouts could double-subscribe (self-harm only).*
- [X] `[HIGH]` **Review finding:** `authorizeBetaSignup` was the second unauthenticated endpoint but had no rate limit, and the access code was hardcoded in a repo whose GitHub mirror (`EMDirks/dingopunks`) is public — the gate was decorative. Fixed: per-IP limit (10 / 10 min, charged before the code comparison so brute force and approval spam hit the same door), code moved to the `BETA_ACCESS_CODE` deploy param (gitignored `.env.dpaam-8864d`), value rotated — the old `BETA01` is burned forever in public git history.
- [X] `[HIGH]` **Review finding (confirmed exploitable, fixed):** the second-from-right `X-Forwarded-For` keying assumed Google appends `<client-ip>, <lb-ip>` — production logging proved gen2 function URLs get exactly **one** appended entry (the client IP, rightmost), so a spoofed header controlled the rate-limit bucket: rotating fake IPs bypassed the limit entirely, and a victim IP's bucket could be charged remotely. Fixed: keying is now rightmost via `GOOGLE_APPENDED_XFF_ENTRIES = 1` with a prod tripwire warning on short chains (see updated "Which IP counts" in §3). Verified end-to-end against production: 32 requests rotating spoofed IPs tripped `resource-exhausted` at exactly the 30-request limit.
- [X] `[HIGH]` **Review finding (2026-09-25, launch-blocking, fixed):** the `resolveGameCode` limit (30 lookups of any kind / 10 min per IP) was sized for attackers, not schools. One school-network IP serves a whole building, so one class entering the correct code could lock out the next student for up to 10 minutes, one student mashing wrong codes could lock out the whole school, and every lookup writing the same counter doc in a transaction made class-sized bursts fail on contention. Fixed: only wrong codes are charged (100 / 1 min), successes only read the counter, a failed counter write still answers "not found", `maxInstances: 5` caps flood cost, and the play page treats a cap-rejected 429 (no `retryAfter`) as a hiccup. Tests cover "successes never count", a 30-request parallel class burst, and "a blocked IP learns nothing".
- [X] `[YOU]` Firestore TTL policy on `betaSignupApprovals.expiresAt` — approvals for emails that never finish signup currently accumulate forever (consume-on-signup deletes the rest). Console setting, same drill as `codes`/`rateLimits`.
- [X] `[LOW]` Optional: per-user rate limit on `createShareCode` (deferred from Phase 2 — reuse the Phase 3 counter infra if the 20-cap proves insufficient).
- [X] `[LOW]` Friendly error toasts for every failure path.
- [X] `[MID]` Email verification — email/password signups get a verification email (with a continue URL back to the account page). Soft gate: unverified users reach the dashboard with a persistent banner (resend with cooldown, change email); only share-code creation waits on `emailVerified`. A background `reload()` watch (poll while visible + on tab focus) flips the UI the moment the link is clicked (`membership/email-verification.js`). Google sign-in is treated as already verified.
- [ ] `[MID]` **Deferred (optional):** Firebase App Check — not required for launch. IP rate limits on `resolveGameCode` / `authorizeBetaSignup` and auth on all other callables are the main controls. Revisit if logs or billing show scripted abuse that limits don't catch (e.g. high `resolveGameCode` volume); roll out monitor-only first, then enforce — watch school-network / ad-blocker false positives.
...
4. **Email verification** — soft gate for email/password accounts: the dashboard opens immediately, share-code creation is withheld client-side until verified, and checkout stays open (Stripe collects the email at checkout; blocking purchase at the post-signup offer step would cost conversions). Google users skip the verify step. Server callables do not yet require `email_verified` in the token; add that to `createShareCode` if client bypass becomes a concern. **Launch:** keep Firebase’s default verify landing page; **post-launch (P1):** custom `auth-action.html` handler — see `readme/dpaam-launch-runway.md` post-launch backlog.
