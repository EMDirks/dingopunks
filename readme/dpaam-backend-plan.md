...
- [X] `[MID]` Email verification — email/password signups get a verification email; dashboard stays gated until `emailVerified` (`membership-auth.js`). Google sign-in is treated as already verified.
...
4. **Email verification** — required for email/password before dashboard access (implemented). Google users skip the verify step. Server callables do not yet require `email_verified` in the token; add that only if client bypass becomes a concern.
