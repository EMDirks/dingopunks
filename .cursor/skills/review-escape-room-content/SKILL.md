...
1. **Spelling** — typos, inconsistent proper nouns for the **same** entity **within one file or challenge** (e.g. title says "Blaze Flower" but the passage uses "Blazeflower"). Do **not** flag when names or locations differ **across grade levels** for the same escape room (e.g. "Grubb" in 3rd grade vs "Grumblegrub" in 5th)—that is intentional grade-band variation.
...
- **Cross-grade name/location variation** — different character names, place names, or villain labels between grade-band files for the same escape room (e.g. shorter nicknames in lower grades, longer names in upper grades) are intentional; do not flag as inconsistent proper nouns.
